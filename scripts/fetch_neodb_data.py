...
def fetch_neodb_data():
    ...
    all_data = {
        'metadata': {
            ...
            'client_id': NEODB_CLIENT_ID  # Add client ID to metadata
        },
        'graph_data': {
            'nodes': [],
            'links': []
    ...
    for shelf_type in shelf_types:
        try:
            ...
            endpoint = f"{BASE_URL}/api/me/shelf/{shelf_type}?category=movie"
            response = requests.get(endpoint, headers=headers)
            
            if response.status_code == 200:
                shelf_data = response.json()
                items = shelf_data.get('data', [])
                ...
                print(f"Failed to fetch {shelf_type} shelf: {response.text[:200]}")
